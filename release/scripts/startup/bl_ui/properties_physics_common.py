...
def point_cache_ui(self, cache, enabled, cachetype):
    ...
    if not cache.use_external or cachetype == 'FLUID':
		...
		# Perfect World : point cache restore button
        sub.operator("ptcache.restore_bake", text="Restore Bake").bake = True

        col = flow.column()
        col.operator("ptcache.bake_all", text="Bake All Dynamics").bake = True
        col.operator("ptcache.free_bake_all", text="Delete All Bakes")
        col.operator("ptcache.bake_all", text="Update All to Frame").bake = False
		# Perfect World : point cache delete manual button
        sub.operator("ptcache.manual_delete_bake", text="Manual Delete Bake")

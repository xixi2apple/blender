          ...
          if (strstr(de->d_name, ext)) {               /* Do we have the right extension? */
            if (STREQLEN(filename, de->d_name, len)) { /* Do we have the right prefix. */
              if (mode == PTCACHE_CLEAR_ALL) {
				...
				// Perfect World : bphys files cache -- comment bphys files delete automatically
...
// Perfect World : bphys files cache -- read baked files to memory
void BKE_ptcache_restore_bake(PTCacheID *pid)
{
  if (pid->cache && pid->type != PTCACHE_TYPE_SMOKE_DOMAIN) {
    ...
    len = ptcache_filename(pid, filename, (int)0, 0, 0); /* no path */
      ...
      if (strstr(de->d_name, ext)) {               /* Do we have the right extension? */
        if (STREQLEN(filename, de->d_name, len)) { /* Do we have the right prefix. */
          ...
          if ((frame != -1) /*&& (frame >= sta && frame <= end)*/) {
            //pid->cache->cached_frames[frame - 0] = 1;
            if (frame > pid->cache->endframe)
              pid->cache->endframe = frame;
          }
        }
      }
    }
    ...
    // pid.cache->endframe = 25;
    BKE_ptcache_disk_to_mem(pid);

    for (int i = 0; i < pid->cache->endframe; i++) {
      pid->cache->cached_frames[i - 0] = 1;
    }
    pid->cache->cached_frames_len = pid->cache->endframe;
    //pid->cache->simframe =
...
// Perfect World : bphys files cache -- clear bake data and delete .bphys files for object 
void BKE_ptcache_delete_bake_manually(PTCacheID *pid)
{
  if (pid->cache && pid->type != PTCACHE_TYPE_SMOKE_DOMAIN) {
    DIR *dir;
    struct dirent *de;

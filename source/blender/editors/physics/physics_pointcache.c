...
// Perfect World : bphys files cache
/* needed for directory lookup for restore bake */
#ifndef WIN32
#  include <dirent.h>
#else
...
// Perfect World : bphys files cache
static bool ptcache_restore_bake_poll(bContext *C)
{
  return CTX_data_scene(C) != NULL;
}

static bool ptcache_manual_delete_bake_poll(bContext *C)
{
  return CTX_data_scene(C) != NULL;
}
...
// Perfect World : bphys files cache -- restore and delete bake execute
static int ptcache_restore_bake_exec(bContext *C, wmOperator *UNUSED(op))
{
  Scene *scene = CTX_data_scene(C);
  PointerRNA ptr = CTX_data_pointer_get_type(C, "point_cache", &RNA_PointCache);
  Object *ob = (Object *)ptr.owner_id;
  ...
  BKE_ptcache_restore_bake(&pid);

  return OPERATOR_FINISHED;
}

static int ptcache_manual_delete_bake_exec(bContext *C, wmOperator *UNUSED(op))
{
  PointerRNA ptr = CTX_data_pointer_get_type(C, "point_cache", &RNA_PointCache);
  Scene *scene = CTX_data_scene(C);
  PointCache *cache = ptr.data;
  ...
  PTCacheID pid = BKE_ptcache_id_find(ob, scene, cache);
  //delete files
  BKE_ptcache_delete_bake_manually(&pid);
  //clear bake
...
// Perfect World : bphys files cache -- restore baked filed correctly
void PTCACHE_OT_restore_bake(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Restore All Physics Bakes";
  ot->idname = "PTCACHE_OT_restore_bake";
  ot->description = "Restore all baked caches of all objects in the current scene";

  /* api callbacks */
  ot->exec = ptcache_restore_bake_exec;
  ot->poll = ptcache_restore_bake_poll;

  /* flags */
  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;
}
// Perfect World : bphys files cache -- delete all baked files manually
void PTCACHE_OT_manual_delete_bake(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Delete All Physics Bakes Manually";
  ot->idname = "PTCACHE_OT_manual_delete_bake";
  ot->description = "Delete all baked caches of all objects in the current scene manually";

  /* api callbacks */
  ot->exec = ptcache_manual_delete_bake_exec;
  ot->poll = ptcache_manual_delete_bake_poll;
